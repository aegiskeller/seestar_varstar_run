...
def setup_argparse():
    ...
    parser.add_argument(
        "exp_time", type=float, help="Time (in seconds) for images in the stack"
    )
    parser.add_argument(
        "session_time", type=float, help="Time (in seconds) for the stacking session"
    )
    parser.add_argument(
        "is_debug",
        type=str,
        default=False,
        nargs="?",
        help="Print debug logs while running.",
    )
    return parser
